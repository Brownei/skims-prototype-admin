...
const ClientMenu: FC<StyleColumnProps> = ({styles}) => {
    const router = useRouter()
    const searchParams = useSearchParams()
    const {onChange} = useInitialStyleStore()
    ...
    async function onDelete(style: string) {
        setIsLoading(true)
        try {
          await axios.delete(`/api/styles/${style}`)
          window.location.assign('/dashboard/styles')
          toast({
            title: `Succesfully deleted ${style}`
          })
        } catch (error) {
    ...
    <div>
      <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => onChange(styles)} className="flex items-center gap-1">
      ...
      <Modal>
        <div>
          ...
          <div className="flex justify-end items-center gap-4 mt-4">
            <Button disabled={isLoading} onClick={() => router.back()} className="mt-2 sm:mt-0" variant='outline'>
              No, I do not think so
            </Button>
            <Button disabled={isLoading} onClick={() => onDelete(search)} variant='destructive'>
              Yes, I am
            </Button>
          </div>
        </div>
