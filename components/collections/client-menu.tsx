...
const ClientMenu: FC<CollectionColumnProps> = ({collections}) => {
    ...
    async function onDelete(collection: string) {
        try {
          ...
          toast({
            title: `Succesfully deleted ${collection}`
          })
        } catch (error) {
    ...
    <div>
      <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => router.push(`/dashboard/collections/${collections.name}`)} className="flex items-center gap-1 font-ProMedium">
              ...
              <DropdownMenuItem asChild className="flex items-center gap-1 font-ProMedium">
                <Link href={`?collection=${collections.name}`}>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-4 h-5">
