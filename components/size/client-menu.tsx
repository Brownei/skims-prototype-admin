...
const ClientMenu: FC<SizeColumnProps> = ({sizes}) => {
    const router = useRouter()
    const searchParams = useSearchParams()
    const params = useParams()
    const search = searchParams.get('size')

    if(params) {
      search
    }

    async function onDelete(size: string) {
        try {
          await axios.delete(`/api/sizes/${size}`)
          toast({
            title: `Succesfully deleted ${size}`
          })
          window.location.assign('/dashboard/sizes')
        } catch (error) {
          console.log('[DELETING SIZE]', error)
          toast({
    ...
    <div>
      <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => router.push(`/dashboard/styles/${sizes.name}`)} className="flex items-center gap-1">
